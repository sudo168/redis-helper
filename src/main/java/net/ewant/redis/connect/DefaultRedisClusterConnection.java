...
public class DefaultRedisClusterConnection extends AbstractRedisConnection implements RedisClusterConnection {
	
	JedisCluster sources;
	
	private boolean closed;

	private int db;

	public DefaultRedisClusterConnection(RedisConnectionFactory factory, JedisCluster jedisCluster) {
		super(factory, jedisCluster);
		this.sources = jedisCluster;
	}
	...
	public void close() {
		this.closed = true;
		// NOOP 
		// the real source Jedis already return to pool 
		// @see redis.clients.jedis.JedisClusterCommand.releaseConnection(Jedis connection)
		// call JedisCluster.close() will destroy all pools
		// @see redis.clients.jedis.JedisClusterInfoCache.reset()
	}
	...
	public String auth(String password) throws Exception {
		throw new IllegalAccessException("not support command [password] in " + sources.getClass().getName());
	}
	...
	public Map<String, List<String>> scan(String cursor) throws Exception {
		throw new IllegalAccessException("not support command [scan(String cursor)] in " + sources.getClass().getName());
	}
