...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    private void initPool(List<HostAndPort> masters) {
    	if (!equals(currentHostMasters, masters)) {
    		...
    		initPool(poolConfig, new ShardedJedisFactory(this, shardMasters, Hashing.MURMUR_HASH, null));
		    	...
		    	if (null == master) {
		    		try {
						log.severe("All sentinels down, cannot determine where is "
							+ masterName + " master is running... sleeping 3000ms, Will try again.");
						Thread.sleep(3000);
    ...
    protected static class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
		...
		private Pool<ShardedJedis> sources;
	
		public ShardedJedisFactory(Pool<ShardedJedis> sources, List<JedisShardInfo> shards, Hashing algo, Pattern keyTagPattern) {
		...
		public PooledObject<ShardedJedis> makeObject() throws Exception {
		    ...
		    jedis.setDataSource(this.sources);
		...
		public void destroyObject(PooledObject<ShardedJedis> pooledShardedJedis) throws Exception {
		    ...
		    shardedJedis.setDataSource(null);
		}
